...
export async function GET(request: NextRequest) {
    const [exciseProducts, exciseVendors, dbProducts, dbVendors] =
        await Promise.all([
            fetchBrands(),
            fetchVendors(),
    ...
    const missingVendors = exciseVendors.filter(
        (v) => !dbVendors.find((dv) => dv.externalId === v.vendId)
    )
    const missingProducts = exciseProducts.filter(
        (v) => !dbProducts.find((dv) => dv.externalKey === v.brandKey)
    )
    await Promise.all([

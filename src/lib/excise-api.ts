...
export function createVendorsByProductRequest(product: Product) {
    return new Request(
        'https://delhiexcise.gov.in/api/BrandAPI/searchBrandAvailabilityByKey',
        {
            headers: {
...
export function mapExciseProduct(
    product: ExciseApiBrand
): Omit<Product, 'createdAt' | 'updatedAt' | 'id' | 'image'> {
    return {
...
export function mapExciseVendor(
    vendor: ExciseApiVendor
): Omit<
    Vendor,
    'createdAt' | 'updatedAt' | 'id' | 'location' | 'gmapsPlaceId'
> {
...
export async function fetchVendors(): Promise<ExciseApiVendor[]> {
    return fetch(
        'https://delhiexcise.gov.in/api/RetailVendAPI/searchRetailVendByAddress',
        {
            headers: {
                'Auth-Token': 'fb5bc83d-94ce-41c2-b9bb-9f198da0ff7d',
            },
